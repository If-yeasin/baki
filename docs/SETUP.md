...
- [ ] `supabase db push` against the live project — migrations `0001`, `0002_rls_hardening`, `0003_balances_helper` applied.
...
- **Account deletion (Apple Guideline 5.1.1(v)):** the in-app stub currently surfaces "coming soon," which is **fine for TestFlight beta** (Apple does not require the deletion flow for internal/external TestFlight builds) but **must ship before public App Store submission**. Timeline: implement a real deletion flow during the TestFlight beta window. Reviewer plan: once shipped, the Settings → "Delete account" entry calls a Supabase Edge Function that revokes sessions, cascade-deletes the user's row from `public.profiles` (which cascades through `group_members`, expenses authored by the user are reassigned to a tombstone "deleted user" profile to preserve other members' ledgers), and signs the client out. Demo path for the App Store reviewer: sign in with the test account → Settings → Delete account → confirm; the account is removed within 30 days per Apple's policy. See "Account deletion" in docs/DATA_MODEL.md for the cascade strategy and Edge Function contract.
